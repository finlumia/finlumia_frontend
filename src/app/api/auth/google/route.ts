...
export async function POST(req: NextRequest) {
  ...
  if (!backendRes.ok) {
    ...
    const message = (err?.message ?? "Não foi possível entrar com o Google") as string;
  ...
  const response = NextResponse.json({ user: data.user ?? null, isNewUser: data.isNewUser ?? false });
